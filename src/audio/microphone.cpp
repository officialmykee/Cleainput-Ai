...
Microphone::Microphone(const MicConfig& cfg)
    : cfg_(cfg)
    , ring_buffer_(
        cfg.sample_rate * cfg.ring_buffer_seconds)
    , running_(false)
    , frames_captured_(0)
    , overruns_(0) {

    frame_samples_ = static_cast<size_t>(
        cfg_.sample_rate * cfg_.frame_ms / 1000.0f);
...
void Microphone::push_audio(
    ...
    frames_captured_.fetch_add(
        count, std::memory_order_relaxed);

    // Fire frame callback when enough samples ready
    if (callback_) {
...
void Microphone::push_audio_int16(
...
bool Microphone::start() {
    ...
    if (res != AAUDIO_OK) {
        std::cerr << "[MIC] AAudio builder failed: "
    ...
    AAudioStreamBuilder_setDirection(
        builder, AAUDIO_DIRECTION_INPUT);
    ...
    AAudioStreamBuilder_setInputPreset(
        builder, AAUDIO_INPUT_PRESET_VOICE_RECOGNITION);
    AAudioStreamBuilder_setPerformanceMode(
        builder, AAUDIO_PERFORMANCE_MODE_LOW_LATENCY);
    AAudioStreamBuilder_setDataCallback(
        builder, aaudio_callback, this);
    ...
    if (res != AAUDIO_OK) {
        std::cerr << "[MIC] AAudio open failed: "
                  << AAudio_convertResultToText(res)
                  << "\n";
        return false;
    }
    ...
    res = AAudioStream_requestStart(
        platform_->stream);
    if (res != AAUDIO_OK) {
        std::cerr << "[MIC] AAudio start failed: "
                  << AAudio_convertResultToText(res)
                  << "\n";
...
static OSStatus audio_unit_callback(
    ...
    std::vector<float> tmp(
        inNumberFrames * mic->cfg_.channels);
    buf_list.mBuffers[0].mData = tmp.data();
...
bool Microphone::start() {
    ...
    if (!comp) {
        std::cerr
            << "[MIC] No RemoteIO component\n";
        return false;
    }

    OSStatus status =
        AudioComponentInstanceNew(
            comp, &platform_->audio_unit);
    if (status != noErr) {
        std::cerr << "[MIC] AudioUnit init failed: "
                  << status << "\n";
        return false;
    }
    ...
    if (status != noErr) {
        std::cerr
            << "[MIC] AudioUnit initialize failed: "
            << status << "\n";
        return false;
    }
